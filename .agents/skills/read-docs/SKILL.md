...
2. **Processed Markdown for Docs Pages**
   Fumadocs templates often expose processed Markdown (plain Markdown derived from MDX) at `/llms.mdx/docs/<slugs>/content.md` for each page.
   Some sites also rewrite `/docs/.../page.mdx` or negotiate `Accept: text/markdown` to that URL via middleware.
   Example:
   ...
   Older sites may still serve raw MDX by appending `.mdx` to the docs path. Try the `content.md` route first, then fall back.
...
- When retrieving page content, prefer `/llms.mdx/.../content.md` (or `Accept: text/markdown` negotiation) over rendered HTML.
- When the user needs to find something (keyword, feature, concept), use the `/api/search` endpoint with a precise query.
- Parse and reason over the Markdown or JSON results to provide accurate, up-to-date answers.
- Cite the exact source URL (prefer the original rendered URL for user readability, but base your understanding on the Markdown version).
